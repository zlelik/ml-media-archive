...
const modelInfos = [
  {
    ...
    loadModel: async function () {
      logMsg(`Start loading model ${this.name}. this.isLoaded: ${this.isLoaded}. this.isSelected: ${this.isSelected}`);
      this.detector = null;
      if ((window.pipeline) && (!this.isLoaded) && (this.isSelected)) {
        logMsg("window.pipeline is defined");
      
        try {
          logMsg(`Try to load the model ${this.name} with webgpu engine`);
          this.detector = await pipeline('object-detection', this.url, { device: 'webgpu' });
          this.isLoaded = true;
          logMsg(`The model ${this.name} is loaded with webgpu engine`);
          $("#backend_used").html("webgpu");
        } catch (err) {
          try {
            logMsg(`Load the model ${this.name} with webgpu engine failed with the error: ${err.message}`);
            logMsg(`Try to load the model ${this.name} with wasm engine`);
            this.detector = await pipeline('object-detection', this.url, { device: 'wasm' });
            this.isLoaded = true;
            logMsg(`The model ${this.name} is loaded with wasm engine`);
            $("#backend_used").html("wasm");
          } catch (errInt) {
            logMsg(`Error while loading the model ${this.name}. error message: ${errInt.message}`, errInt, true);
            throw errInt;
          }
        }
        logMsg(`model ${this.name} is loaded`);
      } else {
        logMsg("window.pipeline is not defined. Model is not loaded.");
      }
    },
    
    processing: async function (img, modelMinProbability) {
  ...
  {
      name: "Xenova_owlvit-base-patch32",
      label: "OWL-ViT - 1203 labels (~615 MB)",
      ...
      loadModel: async function () {
        ...
        if ((window.pipeline) && (!this.isLoaded) && (this.isSelected)) {
          ...
          try {
            logMsg(`Try to load the model ${this.name} with webgpu engine`);
            this.detector = await pipeline('zero-shot-object-detection', this.url, {
              //dtype: 'fp16',
            ...
            try {
              logMsg(`Load the model ${this.name} with webgpu engine failed with the error: ${err.message}`);
              logMsg(`Try to load the model ${this.name} with wasm engine`);
              this.detector = await pipeline('zero-shot-object-detection', this.url, { device: 'wasm' });
              logMsg(`The model ${this.name} is loaded with wasm engine`);
              $("#backend_used").html("wasm");
            } catch (errInt) {
              logMsg(`Error while loading the model ${this.name}. error message: ${errInt.message}`, errInt, true);
          ...
          const enrichedLabelsInfo = this.enrichLabels(this.labelsData);
          this.candidateLabels = enrichedLabelsInfo.candidateLabels;
          this.reverseMap = enrichedLabelsInfo.reverseMap;
          this.isLoaded = true;
          logMsg(`model ${this.name} is loaded`);
        } else {
          logMsg("window.pipeline is not defined. Model is not loaded.");
      ...
      mapProbabilityToThreshold: function(probability) {
        probability = Math.max(0, Math.min(1, probability));

        // Quadratic mapping: threshold = a * x^2 + b * x + c
        const a = 0.45;
        const b = 0.05;
        const c = 0.02;

        return a * probability * probability + b * probability + c;
      },
      
      mapThresholdToProbability: function(threshold) {
        return Math.pow(threshold, 0.45)
      },
      
      enrichLabels: function(labelsData) {
        const candidateLabelsSet = new Set();
        const reverseMap = {};

        labelsData.forEach(label => {
          label.synonyms.forEach(synonym => {
            const prettySyn = synonym.replace(/_/g, ' ');
            const otherSyns = label.synonyms
              .filter(s => s !== synonym)
              .map(s => s.replace(/_/g, ' '))
              .join(', ');

            const variants = [
              `A photo of a ${prettySyn}, ${label.def}`,
              //`An image of a ${prettySyn}, which is defined as ${label.def}`,// commented because was not in use in test runs
              `${prettySyn} — ${label.def}`,
              //`An object known as ${prettySyn}, used as ${label.def}`,// commented because was not in use in test runs
              otherSyns
                ? `A photo of a ${prettySyn} (also called ${otherSyns})`
                : `A photo of a ${prettySyn}`
            ];

            variants.forEach(variant => {
              if (!candidateLabelsSet.has(variant)) {
                candidateLabelsSet.add(variant);
                reverseMap[variant] = label.name.replace(/_/g, ' ').replace(/^\w/, c => c.toUpperCase());
              }
            });
          });
        });

        const candidateLabels = Array.from(candidateLabelsSet);
        return { candidateLabels, reverseMap };
      },
      
      deduplicateSimilarBoxes: function(results, tolerance = 5) {
        const resultsToKeep = [];

        for (const item of results) {
          let areBoxesSimilar = false;

          for (let i = 0; i < resultsToKeep.length; i++) {
            areBoxesSimilar = this.checkSimilarBoxes(item.box, resultsToKeep[i].box, tolerance);
            if (areBoxesSimilar) {
              // Keep the one with the higher score
              if (item.score > resultsToKeep[i].score) {
                resultsToKeep[i] = item;
              }
              break;
            }
          }
      ...
      checkSimilarBoxes: function(box1, box2, tolerance = 5) {
        return (
          Math.abs(box1.xmin - box2.xmin) <= tolerance &&
          Math.abs(box1.ymin - box2.ymin) <= tolerance &&
          Math.abs(box1.xmax - box2.xmax) <= tolerance &&
          Math.abs(box1.ymax - box2.ymax) <= tolerance
        );
      }
    },
    {
      name: "onnx-community_Florence-2-base",
      label: "Florence 2 - Description (~646 MB)",
      url: "onnx-community/Florence-2-base",
      model: null,
      processor: null,
      tokenizer: null,
      task: null,
      ...
      loadModel: async function () {
        ...
        if ((window.pipeline) && (!this.isLoaded) && (this.isSelected)) {
          ...
          this.tokenizer = null;
          this.prompts = null;
          this.text_inputs = null;
          try {
            logMsg(`Try to load the model ${this.name} with webgpu engine`);
            this.model = await Florence2ForConditionalGeneration.from_pretrained(this.url, {
              dtype: {
            ...
            try {
              logMsg(`Load the model ${this.name} with webgpu engine failed with the error: ${err.message}`);
              logMsg(`Try to load the model ${this.name} with wasm engine`);
              this.model = await Florence2ForConditionalGeneration.from_pretrained(this.url, { device: "wasm" });
              logMsg(`The model ${this.name} is loaded with webgpu engine`);
              $("#backend_used").html("wasm");
            } catch (errInt) {
              logMsg(`Error while loading the model ${this.name}. error message: ${errInt.message}`, errInt, true);
          ...
          this.processor = await AutoProcessor.from_pretrained(this.url);
          this.tokenizer = await AutoTokenizer.from_pretrained(this.url);

          // Specify task and prepare text inputs
          this.task = '<MORE_DETAILED_CAPTION>';//supported values: <DETAILED_CAPTION>, <CAPTION>, <MORE_DETAILED_CAPTION>, <OCR>, <OCR_WITH_REGION>
          this.prompts = this.processor.construct_prompts(this.task);
          this.text_inputs = this.tokenizer(this.prompts);
          
          logMsg("Tokenizer is loaded");
          this.isLoaded = true;
          logMsg(`model ${this.name} is fully loaded with tokenizer and processor`);
        } else {
          logMsg("window.pipeline is not defined. Model is not loaded.");
        }
      },
      
      processing: async function(img) {
        logMsg(`Start processing for model ${this.name}`);
...
$(document).ready(async function() {
  ...
  for (let i = 0; i < modelInfos.length; i++) {//using access by index because order is important
    modelsListElement.append(`<label for="${modelInfos[i].name}">${modelInfos[i].label}</label><input type="checkbox" name="${modelInfos[i].name}" id="${modelInfos[i].name}">&nbsp;`);
  ...
  $("#start_btn").click(async function() {
    ...
    const selectedModels = $("#ml_models input[type='checkbox']:checked").map(function() {
       return this.id;
     }).get();
    logMsg("selectedModels: ", selectedModels);
...
const loadSettingsFromStorage = () => {
  try {
    ...
    if (savedSettings) {
      ...
      const selectedModels = savedSettings.selected_models?.split(',');
      modelInfos.forEach(obj => { obj.isSelected = false; });
      modelInfos.forEach(obj => { obj.isSelected = selectedModels?.includes(obj.name) ?? false; });
      
      $("#ml_models input[type='checkbox']").map(function() {
        let inputElement = $(this);
        let id = this.id;
        $(this).prop("checked", selectedModels?.indexOf(id) > -1 ?? false);
        $(this).checkboxradio("refresh");
      });
    ...
    logMsg("Error during setting load: ", err);
  }
};
...
const saveSettingsToStorage = () => {
  if ($("#save_settings").is(":checked")) {
    const settings = {
      selected_models: modelInfos.filter(obj => obj.isSelected).map(obj => obj.name).join(','),
...
const processImage = async (file, fileData, minProbability, ocrEnabled, addPreview, extractExif, isImage) => {
  ...
  const previewCurrentImageInfo = convertImageToBase64WithResize(img, previewSize);
  const previewWidth = previewSize;
  $("#current_img").html(`<img src="${previewCurrentImageInfo.dataURL}" width="${previewWidth}px">`);
  updateCurrentOperation(`${operationPrefix}Preview Generation is finished`);

  // Step 2: Optional save low-resolution preview
  if (addPreview) {
    fileData.previewData = previewCurrentImageInfo.dataURL;
  }
  ...
  // Step 7: Clean up the URL object
...
async function detectObjects(imageInfo, minProbability, fileData) {
  ...
  const minProbabilityLocal = minProbability / 100.0;
  logMsg("detectObjects started");
  
  for (let i = 0; i < modelInfos.length; i++) {
    logMsg("***********************************************************************************************");
    const model = modelInfos[i];
    try {
      if ((model.isSelected) && (model.isObjectDetectionModel)) {
        logMsg(`Object detection started with model ${model.name} and filename: ${fileName}.`);
        updateCurrentOperation(`${operationPrefix}Objects Detection with model ${model.label}`);
        let prediction = await model.processing(imageInfo.dataURL, minProbabilityLocal);
        prediction = convertDetections(prediction, model.name, width, height);
        predictions.push(...prediction);
        //output should be in this format: {"label":"bowl","probability":0.7902,"box":{"ymin":0.3015,"xmin":0.2366,"ymax":0.4948,"xmax":0.5259},"modelName":"EfficientDet"}
        updateCurrentOperation(`${operationPrefix}Objects Detection with model ${model.label} is finished`);
        logMsg(`Object detection finished with model ${model.name} and filename: ${fileName}.`);
        logMsg(`---------------------------Object detection SUCCESS file: ${fileName}-------------------------------`);
      } else {
        logMsg(`Object detection skipped with model ${model.name} and filename: ${fileName}.`);
      }
    } catch (err) {
     logMsg(`Error during object detection on an image ${fileName}. Error message: `, err.message, true);
     logMsg(`Error during object detection on an image ${fileName}. Error object: `, err, true);
     predictions.push({"label":`Error during objects detection: ${err.message} - ${model.label}`,"probability":0.0,"box":{"ymin":0.0,"xmin":0.0,"ymax":0.0,"xmax":0.0},"modelName":model.label});
   } finally {
   }
   logMsg("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
  }
  ...
  /*for (const [modelName, model] of Object.entries(loadedModels)) {
    if (selectedModels.indexOf(modelName) > -1) {
      ...
      try {
        logMsg(`Object detection started with model ${modelName}.`);

        logMsg(`Object detection finished with model ${modelName}.`);
        logMsg(`---------------------------Object detection SUCCESS file: ${fileName}-------------------------------`);
      } catch (err) {
        logMsg(`Error during object detection on an image ${fileName}. Error message: `, err.message, true);
        logMsg(`Error during object detection on an image ${fileName}. Error object: `, err, true);
...
function convertDetections(inputArray, modelName, imageWidth, imageHeight) {
  return inputArray.map(item => {
    return {
      label: item.label,
      probability: Number(item.score.toFixed(4)),
      box: {
        ymin: +(item.box.ymin / imageHeight).toFixed(4),
        xmin: +(item.box.xmin / imageWidth).toFixed(4),
        ymax: +(item.box.ymax / imageHeight).toFixed(4),
        xmax: +(item.box.xmax / imageWidth).toFixed(4)
      },
      modelName: modelName
    };
  });
...
async function getImageDescription(imageDataURL, minProbability, fileName, isImage) {
  ...
  if (isImage) {
    for (let i = 0; i < modelInfos.length; i++) {
        ...
        const model = modelInfos[i];
        try {
          if ((model.isSelected) && (model.isImageCaptioning) && (!result)) {
            logMsg(`Image description started with model ${model.name} for the file ${fileName}.`);
            updateCurrentOperation(`${operationPrefix}Image Description Generation with model ${model.label}`);
            result = await model.processing(imageDataURL);
            updateCurrentOperation(`${operationPrefix}Image Description Generation with model ${model.label} is finished`);
            logMsg(`Image description finished with model ${model.name} for the file ${fileName}.`);
          } else {
            logMsg(`Image description skipped with model ${model.name} for the file ${fileName}.`);
          }
        } catch (err) {
         logMsg(`Error during image description generation with the model ${model.label} on an image ${fileName}. Error message: `, err.message, true);
         logMsg(`Error during image description generation with the model ${model.label} on an image ${fileName}. Error object: `, err, true);
         result = `[Error during image description generation: ${err.message}]`;
       } finally {
       }
      }
    } else {
      logMsg("File is not an image. Only image description is supported.");
    }
    logMsg("Image description generation finished");
    return result;
}
...
function convertImageToBase64WithResize(img, maxDimension = 0) {
  ...
  return result; // Return the base64-encoded preview
};
...
async function resizeImage(img, maxDimension) {
  ...
  if (currentMaxDimention > maxDimension) {
    ...
    let resizedImageInfo = convertImageToBase64WithResize(img, maxDimension);
    resizedImage.src = resizedImageInfo.dataURL;
...
async function withModelRetry(model, fn, retries = 1) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      logMsg(`Try to run the model ${model.name} processing [attempt: ${attempt}].`);
      const result = await fn();
      logMsg(`The model ${model.name} processing is finished successfully [attempt: ${attempt}].`);
      return result;
    } catch (err) {
      if (err.message.includes("Session mismatch") && attempt < retries) {
        logMsg(`Session mistmatch error happend with the model ${model.name}. Try to reload the model and try again.`);
        console.warn(`Session mismatch with the model ${model.name}. Reloading model...`);
